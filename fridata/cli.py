...
from fridata.models.manage_dataset.utils import install_afdb_download_prefilter
...
from fridata.scripts.command_parser import CommandParser
...
def create_parser():
    ...
    create_archive_parser = subparsers.add_parser(
        ...
        epilog=(
            "Working directory: pass an absolute -p/--file-path to the dataset "
            "directory (the folder containing dataset.json), or cd to "
            "<data_path>/datasets first if you want to use a short folder name.\n"
            "Invocation: run `fridata create_archive ...`, or "
            "`python -m fridata create_archive ...` if the console script is not "
            "on PATH."
        ),
    )
...
def main():
    ...
    from fridata.config import load_config
    try:
