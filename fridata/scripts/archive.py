...
from fridata.models.manage_dataset.utils import read_all_pdbs_from_h5, read_pdbs_from_h5
...
def normalize_pdb_codes_for_h5(codes: list[str]) -> list[str]:
    ...
    ``pdbs.h5`` semicolon keys may appear with or without a ``.pdb`` suffix
    depending on pipeline (e.g. AFDB uses ``ACCESSION_chain``; some writers
    use ``*.pdb``). Include both forms so filtering hits the stored name.
    """
    out: list[str] = []
    seen: set[str] = set()
    for c in codes:
...
def process_h5_file(
    ...
    When ``pdb_codes`` is set, only those entries are read (subset of a shard).
    When every requested code is missing or the H5 cannot be read, returns
    ``None`` so the caller can skip this shard (no empty zip).

    When ``pdb_codes`` is ``None``, exports every structure in the shard
    (legacy behavior).
    """
...
def create_archive(
    structures_dataset: "StructuresDataset", keep_shard_zips: bool = False
):
    ...
    futures = []
    for h5_file, raw_ids in proteins_index.items():
